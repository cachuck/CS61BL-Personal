public class BinaryTree<T> {
    ...
    public static BinaryTree<Integer> fibTree(int N) {
        ...
        if (N == 0){
            ...
            return new BinaryTree(zero);
        }
        if (N == 1){
            TreeNode<Integer> one = new TreeNode<>(1);
            return new BinaryTree(one);
        }
        TreeNode<Integer> parent = new TreeNode<>(fibTree(N-1).root.item + fibTree(N-2).root.item, fibTree(N - 1).root, fibTree(N - 2).root);
        return new BinaryTree(parent);
    }

    /*
    if N = 0 or 1 return tree of 1/0
    helper:
    if node.right = 0 return
    node = N
    node.left = fib(n-1)
    node.right = fib(n-2)
     */
