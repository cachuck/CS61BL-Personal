...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
            for (Node m = sentinel.next; m != sentinel; m = m.next){
                if (item.compareTo(m.item) < 0){
                    Node j = new Node(item, m.prev, m);
                    m.prev.next = j;
                    m.prev = j;
                    return;
    ...
    public DLList<T> mergeSort() {
        ...
        // =================== BEGIN MERGE ===================
        int middle =  this.size/2; //Finds midpoint of DLL
        Node i = sentinel.next; //Sets i to be the first item in the DLL
        for (int m = 0; m < middle; m++){ //Adds the first half of the given DLL to oneHalf
            oneHalf.addLast(i.item);
            i = i.next; //Sets i to next node in the DLL
        }
        for (int m = middle; m < this.size; m++){ //Adds the second half of the given DLL to otherHalf
            otherHalf.addLast(i.item);
            i = i.next; //Sets i to next node in the DLL
        }
    ...
    public DLList<T> quicksort() {
        ...
        for (int m = 0; m < this.size; m++) {
            if (i.item.compareTo(pivot) > 0) {
                largeElements.addLast(i.item);
                i = i.next;
            }
            else if (i.item.compareTo(pivot) < 0) {
                smallElements.addLast(i.item);
                i = i.next;
            }
            else {
                equalElements.addLast(i.item);
                i = i.next;
            }
        }
